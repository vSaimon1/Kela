...
function HomePage({ c }: { c: Copy }) {
  ...
  return <main>
    ...
    <div className="ticker" aria-hidden="true"><div className="ticker-track">{[0, 1].map((group) => <span className="ticker-item" key={group}>{c.home.ticker}</span>)}</div></div>
    <section className="manifesto"><div className="container-wide manifesto-grid"><div className="section-label reveal">02 / KELA</div><div className="reveal delay-1"><h2 className="display">{c.home.manifesto.split(' ').map((word, i) => <span key={word + i}>{i > 3 ? <em>{word} </em> : `${word} `}</span>)}</h2><p className="manifesto-sub">{c.home.manifestoText}</p></div></div></section>
